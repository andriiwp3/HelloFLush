...
function forms() {
	...
	$.each($('input.form__input_tel'), function (index, val) {
		$(this).attr('type', 'tel');
		$(this).inputmask('+55(999) 999-99-99', {
			clearIncomplete: false, clearMaskOnLostFocus: false
		});
		$(this).focus(function () {
			$(this).addClass('focus');
			$(this).parent().addClass('focus');
...
$('form button[type=submit]').click(function (e) {
	...
	if (er == 0) {
		...
		function showStatus(response, form, section) {
			if ((response.status / 2).toString().split('')[0] != '1') {
				section.find('.feedback__reply').addClass('err');
			}
			form.css('transform', 'scale(0)');
			setTimeout(function () {
				form.hide();
			}, 300);
			section.find('.feedback__reply').show();
			setTimeout(function () {
		...
		form.find('input').each(function () {
			var th = $(this);
			if (th.val() !== '') {
				if (th.attr('name') == 'email' || th.hasClass('form__input_email')) {
					arr.email = th.val();
				} else if (th.attr('name') == 'tel' || th.hasClass('form__input_tel')) {
					if (th.inputmask("isComplete")) {
						arr.phone = th.val();
					}
...
function formValidate(input) {
	...
	if (input.attr('name') == 'email' || input.hasClass('form__input_email')) {
